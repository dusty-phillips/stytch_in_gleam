...
type Model {
  Model(stytch_ui_model.AuthModel(cats.CatCounter))
}

fn init(_args) -> #(Model, Effect(Msg)) {
  let api_url = "http://localhost:3000/api"
  #(
    Model(stytch_ui_model.new("http://localhost:3000/api")),
...
fn update(model: Model, msg: Msg) -> #(Model, Effect(Msg)) {
  case model, msg {
    Model(auth_model), AuthMsg(auth_msg) -> {
      let #(model, effect) =
        stytch_ui_model.update_auth(auth_model, auth_msg, cats.init_cat_counter)
      #(Model(model), effect.map(effect, AuthMsg))
    }
    Model(stytch_ui_model.AuthModel(
      api_url,
      stytch_ui_model.Authenticated(email:, model: cat_counter),
    )),
      CatMsg(cat_msg)
    -> {
      ...
      #(
        Model(stytch_ui_model.AuthModel(
          api_url,
          stytch_ui_model.Authenticated(email, model),
        )),
    ...
    Model(stytch_ui_model.AuthModel(state: stytch_ui_model.Authenticating, ..)),
      _
    -> #(model, effect.none())
    Model(stytch_ui_model.AuthModel(
      state: stytch_ui_model.Unauthenticated(_),
      ..,
    )),
      _
    -> #(model, effect.none())
...
fn view(model: Model) -> Element(Msg) {
  let Model(stytch_ui_model.AuthModel(state:, ..)) = model
  case state {
